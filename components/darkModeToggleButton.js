...
export default function DarkModeToggleButton() {
  ...
  return (
    <button
      className={`
        inline-flex items-center border-0 py-1 px-2 
        focus:outline-none rounded text-base
        bg-gray-100 hover:bg-gray-50 hover:text-orange-500
        dark:bg-slate-600 dark:text-yellow-500 dark:hover:text-yellow-300
        transition-colors duration-200
      `}
      type="button"
      onClick={() => setTheme(isDark ? 'light' : 'dark')}
    >
      {/* 다크 모드일 때: ☀️ 라이트 모드 아이콘 */}
      {isDark ? (
        <svg
        ...
        >
          <path
            ...
            18.75V21m-4.773-4.227l-1.591 1.591M5.25 
